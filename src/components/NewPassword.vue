<template>
	...
	<div class="contenido">
			<div class="w-[80%] m-auto bg-white shadow-md p-6">
					<h2 class="flex flex-col text-left text-[#2A5CAA] font-bold text-2xl mb-6 bg-gradient-to-r from-gray-100 to-[#E5EAFF] p-3">Cambiar Contraseña</h2>

					<div class="flex gap-6">
							<form class="flex flex-col gap-6 w-[50%]" @submit.prevent="changePassword">
									<div class="flex flex-col items-start w-[100%]">
											<label class="block uppercase tracking-wide text-[#193368] text-xs font-bold mb-2" for="newPassword">Nueva Contraseña</label>
											<div class="flex w-[80%] flex-row">
													<div class="w-[100%] max-w-[400px] min-w-[250px]">
															<input class="w-[100%] max-w-[400px] h-8 ml-5 bg-[#F5F5F5] text-gray-700 border border-gray-300 rounded-l-full py-1 px-4 focus:outline-none focus:bg-white focus:border-gray-500" v-model="password.old_password" :type="showOldPassword ? 'text' : 'password'" id="newPassword">
													</div>
													<div class="col">
															<button class="w-[50px] flex items-center justify-center h-8 bg-[#08245B] text-white rounded-r-full p-2" type="button" @click="toggleShowOldPassword">
									...
									<div class="flex flex-col items-start w-[100%]">
											...
											<div class="flex flex-row w-[80%] ">
													<div class="w-[100%] max-w-[400px] min-w-[250px]">
															<input class="w-[100%] max-w-[400px] h-8 ml-5 bg-[#F5F5F5] text-gray-700 border border-gray-300 rounded-l-full py-1 px-4 focus:outline-none focus:bg-white focus:border-gray-500" v-model="password.new_password" :type="showNewPassword ? 'text' : 'password'" id="confirmPassword">
													</div>
													<div class="col">
															<button class="w-[50px] flex items-center justify-center h-8 bg-[#08245B] text-white rounded-r-full p-2" type="button" @click="toggleShowNewPassword">
...
const isOldPasswordValid = computed(() => {
	return passwordRegex.test(password.old_password);
});

// Validar la nueva contraseña
const isNewPasswordValid = computed(() => {
	return passwordRegex.test(password.new_password);
});
...
function changePassword() {
	if (!isOldPasswordValid.value || !isNewPasswordValid.value) {
		alert("Por favor, asegúrate de que las contraseñas cumplan con los requisitos.");
		return;
	}

	else {
		alert("Contraseña cambiada de manera exitosa.")
	}
