<template>
  ...
  <div class="w-[70%] h-[90%] m-auto relative p-3">
    <div class="relative bg-white rounded-lg dark:bg-gray-700 w-[100%] h-[100%] p-8">
      ...
      <div class="relative  p-4">
        ...
        <!-- Tabla -->
        <div class="max-h-[60vh] overflow-y-auto"> <!-- Añadir max-height y overflow-y-auto -->
          <table class="w-full text-sm text-left rtl:text-right text-[#193368] dark:text-gray-400">
            ...
            <tbody v-if="users.length > 0">
              <tr v-for="user in users" :key="user.id_number" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                <td class="w-4 p-4">
                ...
                <td v-if="user.status == 'Pendiente'" class="px-6 py-4">
                  <span class="text-sm text-orange-500">{{ user.status }}</span>
                </td>
                <td v-if="user.status == 'Activo'" class="px-6 py-4">
                  <span class="text-sm text-green-500">{{ user.status }}</span>
                </td>
                <td v-if="user.status == 'Inactivo'" class="px-6 py-4">
                  <span class="text-sm text-red-500">{{ user.status }}</span>
                </td>
              </tr>
            </tbody>
