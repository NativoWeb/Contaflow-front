      ...
      <div class="relative p-5 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
          ...
          <div class="p-6 md:p-5 text-center">
            <img src="@/assets/work_remove.svg" alt="Icono de SVG convertido" class="mx-auto mb-4 w-20 h-20 dark:text-gray-200">
            <h3 class="mb-2 text-xl font-bold text-[#2A5CAA]">Se removio la empresa de manera correcta</h3>
...
<script setup>
  ...
  onMounted(async () => {
    try{
      ...
      companiesList.value += [dataUser.value.companies];
      companies.value = companiesList.value ? companiesList.value.split(",") : [];
    }
    catch(err){
  ...
  const removeClient = () => {
    isLoading.value = true;
    companies.value = companies.value.filter(id => id !== companyId)
    const uniqueCompanies = [...new Set(companies.value)];
