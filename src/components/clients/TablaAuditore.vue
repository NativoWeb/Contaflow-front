<template>
   ...
   <div class="overflow-x-auto p-4 bg-white shadow-md rounded-lg">
    <table class="w-full text-sm text-left text-gray-800 dark:text-gray-400">
        ...
        <tbody v-if="data.auditors.length > 0">
          <tr v-for="auditor in data.auditors_data" :key="auditor.id"  class="cursor-pointer bg-white border-b hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
            <td class="px-6 py-4">{{ auditor.first_name }} {{ auditor.last_name }}</td>
        ...
        <td class="ml-2 my-6">No se encontraron auditores registrados.</td>
      </tr>
    </table>
    </div>
    </div>
