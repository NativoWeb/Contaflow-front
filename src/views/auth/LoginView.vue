      ...
      <div class="login-form">
        ...
        <form @submit.prevent="login">
          ...
          <div :class="['input-group', { 'error': errors.email }]">
            <input v-model="authUser.email" type="text" id="usuario" @input="validateEmail" />
          </div>
          <p v-if="errors.email" class="error-message">{{ errors.email }}</p>

          <label for="password">Contraseña</label>
          <div :class="['input-group', { 'error': errors.password }]">
            <input :type="showPassword ? 'text' : 'password'" v-model="authUser.password" id="password"
              autocomplete="on" @input="validatePassword" />
...
function validateEmail() {
  ...
  } else if (!emailPattern.test(authUser.email)) {
